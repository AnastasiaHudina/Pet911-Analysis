...
def create_visualizations(region_stats, dataset_type, top_regions_count=5, output_prefix=''):
    ...
    plt.tight_layout()
    plt.savefig(f'{output_prefix}requests_by_region_pie.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def analyze_dataset(file_path, dataset_type, top_regions_count=5):
    ...
    top_regions = create_regions_table(region_stats_full, dataset_type, top_regions_count,
                                       output_prefix=f'{dataset_type}_')

    # Создание визуализаций
    create_visualizations(region_stats_viz, dataset_type, top_regions_count, output_prefix=f'{dataset_type}_')
