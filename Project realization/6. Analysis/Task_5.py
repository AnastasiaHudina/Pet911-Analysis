...
if len(valid_data) > 0 and valid_data['статус'].nunique() > 1:
    ...
    plt.title("Влияние скорости публикации на успех (пропажа)")
    plt.ylabel("Время до публикации, дни")
    plt.xlabel("Статус объявления")
    plt.tight_layout()
    plt.savefig('temp_images/1_время_до_публикации_пропажа.png', dpi=150)
    plt.close()
...
if len(age_data) > 0:
    ...
    plt.xlabel("Статус")
    plt.tight_layout()
    plt.savefig('temp_images/2_возраст_пропажа.png', dpi=150)
    plt.close()
...
plt.title("Успешность по типу местности (пропажа)")
plt.ylabel("Доля найденных")
plt.xlabel("Тип местности")
plt.xticks(rotation=0)
plt.tight_layout()
plt.savefig('temp_images/3_тип_местности_пропажа.png', dpi=150)
plt.close()

# 4. Фото
plt.figure(figsize=(8, 6))
photo_success = lost_df.groupby('есть_фото')['статус'].apply(lambda x: (x == 'питомец найден').mean())
photo_success.plot(kind='bar', color=['red', 'green'])
plt.title("Влияние наличия фото (пропажа)")
plt.ylabel("Доля найденных")
plt.xlabel("Наличие фото")
plt.xticks(rotation=0)
plt.tight_layout()
plt.savefig('temp_images/4_наличие_фото_пропажа.png', dpi=150)
plt.close()
...
plt.title("Влияние породистости (пропажа)")
plt.ylabel("Доля найденных")
plt.xlabel("Породистое животное")
plt.xticks(rotation=0)
plt.tight_layout()
plt.savefig('temp_images/5_породистость_пропажа.png', dpi=150)
plt.close()
...
if len(valid_data) > 0 and valid_data['статус'].nunique() > 1:
    plt.figure(figsize=(10, 6))
    sns.boxplot(data=valid_data, x='статус', y='время_до_публикации')
    plt.title("Скорость публикации при находке")
    plt.ylabel("Время до публикации, дни")
    plt.xlabel("Статус")
    plt.tight_layout()
    plt.savefig('temp_images/6_время_до_публикации_находка.png', dpi=150)
    plt.close()
...
plt.title("Успешность по типу местности (находка)")
plt.ylabel("Доля возвратов")
plt.xlabel("Тип местности")
plt.xticks(rotation=0)
plt.tight_layout()
plt.savefig('temp_images/7_тип_местности_находка.png', dpi=150)
plt.close()

# 3. Фото
plt.figure(figsize=(8, 6))
photo_found = found_df.groupby('есть_фото')['статус'].apply(lambda x: (x == 'хозяин найден').mean())
photo_found.plot(kind='bar', color=['gray', 'teal'])
plt.title("Влияние фото на возврат")
plt.ylabel("Доля возвратов")
plt.xlabel("Наличие фото")
plt.xticks(rotation=0)
plt.tight_layout()
plt.savefig('temp_images/8_наличие_фото_находка.png', dpi=150)
plt.close()
...
plt.title("Узнаваемость по породе")
plt.ylabel("Доля возвратов")
plt.xlabel("Породистое животное")
plt.xticks(rotation=0)
plt.tight_layout()
plt.savefig('temp_images/9_породистость_находка.png', dpi=150)
plt.close()
...
print(f"\n✅ Вывод сохранён в report_output.txt")
print("✅ Все графики в папке 'temp_images/'")