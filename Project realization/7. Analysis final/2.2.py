...
def create_photo_success_chart(photo_success, display_name, success_description):
    ...
    plt.savefig(f'Результаты 2 главы анализа/2.2.1. Успешность поиска в зависимости от наличия фото для {display_name}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_photos_count_chart(df_analysis, display_name, success_description):
    ...
    plt.tight_layout(rect=[0, 0.05, 1, 0.95])
    plt.savefig(f'Результаты 2 главы анализа/2.2.2. Успешность поиска в зависимости от количества фото для {display_name}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_description_length_chart(df_analysis, display_name, success_description):
    ...
    plt.tight_layout(rect=[0, 0.05, 1, 0.95])
    plt.savefig(f'Результаты 2 главы анализа/2.2.3. Успешность поиска в зависимости от длины описания для {display_name}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_combined_factors_chart(df_analysis, display_name, success_description):
    ...
    plt.tight_layout(rect=[0, 0.05, 1, 0.95])
    plt.savefig(f'Результаты 2 главы анализа/2.2.4. Успешность поиска в зависимости от комбинированных факторов для {display_name}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    os.makedirs('Результаты 2 главы анализа', exist_ok=True)

    # Анализ датасета найденных животных (поиск хозяина)
    analyze_single_dataset_publication('dataset_final_Pet911_found.csv', 'found')

    # Анализ датасета потерянных животных (поиск питомца)
    analyze_single_dataset_publication('Dataset_final_Pet911_lost.csv', 'lost')
