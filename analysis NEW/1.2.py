...
def create_daily_analysis(daily_data, dataset_type, df, output_prefix=''):
    ...
    plt.tight_layout()
    plt.savefig(f'Результаты 1 главы анализа/1.2.1. Распределение общего количества заявок по дням недели для {output_prefix}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_weekly_analysis(weekly_data, dataset_type, df, output_prefix=''):
    ...
    plt.tight_layout()
    plt.savefig(f'Результаты 1 главы анализа/1.2.2. Распределение общего количества заявок по неделям для {output_prefix}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_monthly_forecast(monthly_data, dataset_type, df, output_prefix=''):
    ...
    plt.tight_layout()
    plt.savefig(f'Результаты 1 главы анализа/1.2.3. Прогноз на основе общего количества заявок по месяцам для{output_prefix}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main_analysis():
    ...
    analyze_dataset('Dataset_final_Pet911_lost.csv', 'lost', output_prefix='lost')

    # Анализ для found датасета (поиск хозяев)
    analyze_dataset('dataset_final_Pet911_found.csv', 'found', output_prefix='found')
