...
os.makedirs('Результаты 1 главы анализа', exist_ok=True)
...
def create_regions_table(region_stats_full, dataset_type, top_regions_count=5, output_prefix=''):
    ...
    plt.tight_layout()
    plt.savefig(f'Результаты 1 главы анализа/1.1.1 Топ-10 регионов по проценту успешных случаев для {output_prefix}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_visualizations(region_stats, dataset_type, top_regions_count=5, output_prefix=''):
    ...
    plt.tight_layout()
    plt.savefig(f'Результаты 1 главы анализа/1.1.2. Распределение количества заявок по регионам для {output_prefix}.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'Результаты 1 главы анализа/1.1.3. Распределение количества заявок по регионам для {output_prefix} (Круговая диаграмма).png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'Результаты 1 главы анализа/1.1.4. Процент успешных случаев поиска по регионам для {output_prefix}.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'Результаты 1 главы анализа/1.1.5. Сравнение общего количества заявок и успешных случаев по регионам для {output_prefix}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main_analysis():
    ...
    df_lost, stats_lost_viz, stats_lost_full = analyze_dataset(
        'Dataset_final_Pet911_lost.csv', 'lost', top_regions_count=5
    )

    # Анализ для found датасета (поиск хозяев)
    df_found, stats_found_viz, stats_found_full = analyze_dataset(
        'dataset_final_Pet911_found.csv', 'found', top_regions_count=5
    )
