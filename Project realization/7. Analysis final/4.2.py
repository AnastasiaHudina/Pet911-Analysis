...
import os  # Добавляем модуль для работы с файловой системой
...
def create_directories():
    ...
    """
    base_dir = "Результаты 4 главы анализа"
...
def find_optimal_clusters(features_scaled, clustering_dir):
    ...
    output_path = os.path.join('Результаты 4 главы анализа/4.2.1. Оптимальное количество кластеров.png')
...
def visualize_clusters_2d(features_scaled, cluster_labels, feature_names, clustering_dir):
    ...
    # Сохраняем в папку кластеризации
    output_path = os.path.join('Результаты 4 главы анализа/4.2.2. Визуализация кластеров.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def visualize_cluster_profiles(cluster_analysis, cluster_names, clustering_dir):
    ...
    radar_path = os.path.join('Результаты 4 главы анализа/4.2.3. Средние значения данных анкет внутри кластеров.png')
    ...
    # Сохраняем bar plot
    bar_path = os.path.join('Результаты 4 главы анализа/4.2.4. Успешность поиска по типам кластеров.png')
    ...
    # Сохраняем heatmap
    heatmap_path = os.path.join('Результаты 4 главы анализа/4.2.5. Тепловая карта кластеров.png')
...
def main():
    ...
    try:
        ...
        print(f"\n💡 Все файлы успешно сохранены в папку: 'Результаты 4 главы анализа'")
